...
const BookSession = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch("/api/v1/student/bookSession",{
        method : "POST",
        headers : {
        ...
        <div className="w-full flex justify-center flex-col items-center bg-[#1F3537] py-10 px-5 rounded-lg">
          ...
          <div className="mt-5 grid grid-cols-1 gap-y-4">
            <div className="flex items-center">
              ...
              <span className="ml-2 text-[#57fdfd]">{teacher.subjectSpecialization}</span>
            </div>
            <div className="flex items-center">
              <span className="sm:text-xl font-medium text-white">
                Education Qualification:
              </span>
              <span className="ml-2 text-[#57fdfd]">{teacher.educationQualification}</span>
            </div>
            <div className="flex items-center">
              <span className="sm:text-xl font-medium text-white">
                Experience:
              </span>
              <span className="ml-2 text-[#57fdfd]">{teacher.experience}</span>
            </div>
          </div>
        </div>
      </div>
      <div className="max-w-2xl lg:w-1/2 w-full">
        <div className="w-full flex justify-center flex-col items-center py-6">
          <form onSubmit={handleSubmit} className="space-y-4 w-4/6">
            ...
            <button
              type="submit"
              className={`w-full  text-white py-2 px-4 rounded-md  ${loading ? "cursor-not-allowed bg-gray-400" : "bg-blue-500 hover:bg-blue-600"}`}
            >
